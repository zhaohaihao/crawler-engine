...
import com.frame.process.utils.excel.api.ExcelApi;
import com.frame.process.utils.excel.api.POIExcelApi;
import com.frame.process.utils.excel.api.callback.AbstractExcelReaderCallBack;
...
public abstract class AbstractExcelReader<T> implements ExcelReader<T> {
    ...
    public List<T> readExcel(InputStream inputStream) throws Exception {
        ...
        ExcelReaderCallBack callBack = new AbstractExcelReaderCallBack() {
            ...
            public void onCell(ExcelReaderCallBack.CellContent content) throws Exception {
                ...
                boolean isHeader = isHeader(content, header, headerLine);
                if (isHeader) {
                    // 如果该行为表头
                    header.add(String.valueOf(content.getValue()));
                    headerLine = content.getRow();
     ...
     * @return 是否为表头
     */
    protected boolean isHeader(ExcelReaderCallBack.CellContent content, List<String> header, int headerLine) {
        return header.size() == 0 ? !"".equals(content.getValue()) :  content.getRow() == headerLine ;
    }
}
